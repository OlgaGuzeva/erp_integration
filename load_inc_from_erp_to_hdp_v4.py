...
def concatenate_rows(fetch_res):
    """
    Данные из метода RODPS_REPL_SOURCE_FETCH возвращаются строками определенной длины.
    Если строка таблицы не поместилась в эту длину,
    то продолжение переносится на следующую строку и значение флага CONTINUATION = Х
...
def save_to_file(ready_df, j):
    local_file_name = '{}/{}_{}.csv'.format(folder, q_name, cur_date)
    hdfs_file_name = '{}/{}_{}_{}.csv'.format(hdfs_folder, q_name, cur_date, str(j))
    ...
    logging.info('Writing file to HDFS: {}'.format(hdfs_file_name))

    client.upload(hdfs_file_name, local_file_name, n_threads=10)

    logging.info('Writing to HDFS is finished. File {}'.format(hdfs_file_name))
    logging.info('Size: {}, {}'.format(str(ready_df.shape[0]), str(ready_df.shape[1])))
...
if __name__ == '__main__':
    ...
    queue_params = {'I_SUBSCRIBER_TYPE': u'BOBJ_DS',
                    ...
                    'I_QUEUENAME': u'{}'.format(sys.argv[1]),
                    'I_EXTRACTION_MODE': u'D'
                    }

    folder = '/airflow/erp/{}'.format(queue_params['I_QUEUENAME'])
    hdfs_folder = 'inc/{}'.format(queue_params['I_QUEUENAME'])
    q_name = queue_params['I_QUEUENAME']
    cur_date = datetime.now().strftime('%Y%m%d')

    # Logging
    logging.basicConfig(format=u'%(filename)s[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s',
                        filename=u'/airflow/erp/logs/{}_{}_log.log'.format(q_name, cur_date),
                        level=logging.INFO)
    ...
    while True:
        ...
        tmp_file_name = 'inc/tmp_{}'.format(queue_params['I_QUEUENAME'])
